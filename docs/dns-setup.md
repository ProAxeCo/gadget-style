# DNS Setup — Connect www.gadgetstyle.com.au to Vercel

The site is live at `gadget-style.vercel.app`. To make
**www.gadgetstyle.com.au** point to it, you need to add two DNS records at
your domain registrar (where you bought `gadgetstyle.com.au`).
...
### Record 1 — root/apex domain (`gadgetstyle.com.au`)
...
This makes `gadgetstyle.com.au` resolve to Vercel. Vercel auto-redirects (HTTP
307) the apex domain to `www.gadgetstyle.com.au`.

### Record 2 — www subdomain (`www.gadgetstyle.com.au`)
...
This makes `www.gadgetstyle.com.au` resolve to Vercel's CDN, which serves the
site.
...
### Cloudflare
1. Log in → pick the `gadgetstyle.com.au` zone
2. **DNS → Records → Add record**
3. First record: Type `A`, Name `gadgetstyle.com.au` (or `@`), IPv4 `216.198.79.1`, **Proxy status: DNS only** (orange cloud OFF — IMPORTANT, otherwise SSL breaks)
4. Second record: Type `CNAME`, Name `www`, Target `c540a0c198c26fda.vercel-dns-017.com`, **Proxy: DNS only**
5. Save

### Namecheap
1. Log in → Domain List → **Manage** next to `gadgetstyle.com.au`
2. **Advanced DNS** tab
...
### GoDaddy
1. My Products → DNS next to `gadgetstyle.com.au`
2. **Add New Record**
...
1. Log in → Manage Domain → `gadgetstyle.com.au`
...
1. Log in → DNS for `gadgetstyle.com.au`
...
2. Click **Refresh** next to each domain. When ready, the warning badge
   ...
   `https://www.gadgetstyle.com.au`.
3. Vercel auto-issues a Let's Encrypt SSL cert at this point (no manual steps).

**Or check from a terminal:**
```bash
nslookup www.gadgetstyle.com.au 8.8.8.8
nslookup gadgetstyle.com.au 8.8.8.8
```
The CNAME / A record results should match the Vercel values.

**Or use a DNS checker:** https://dnschecker.org/#A/gadgetstyle.com.au

---

## What to expect when DNS is live

- `https://www.gadgetstyle.com.au` — serves the site
- `https://gadgetstyle.com.au` — 307 redirect to www
- `http://www.gadgetstyle.com.au` — auto-upgraded to HTTPS by Vercel
...
1. **Update Pinterest profile website field** (`https://www.gadgetstyle.com.au`) — Pinterest may have it stuck on `http://` until the site is reachable on HTTPS. Re-save once it is.
2. **Update Instagram website field** (mobile app only — IG blocks website edit on desktop): `https://www.gadgetstyle.com.au`
3. **Verify Pinterest pin click-throughs** by visiting one of your pins and confirming it lands on a real product page.
4. **Add `www.gadgetstyle.com.au` to Google Search Console** for SEO indexing.
